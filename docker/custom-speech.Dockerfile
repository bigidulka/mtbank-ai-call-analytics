...
RUN if [ "$USE_WHEELHOUSE" = "1" ]; then \
        ...
        echo "USE_WHEELHOUSE must be 0 or 1" >&2 && exit 2; \
    fi \
...
USER 10001:10001
EXPOSE 8010
CMD ["uvicorn", "services.custom_speech.app:create_app", "--factory", "--host", "0.0.0.0", "--port", "8010", "--workers", "1", "--log-level", "warning", "--no-access-log"]
