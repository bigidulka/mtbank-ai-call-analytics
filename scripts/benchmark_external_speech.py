...
def _transcribe(client: httpx.Client, endpoint: str, audio: Path, timeout_seconds: float) -> tuple[str, float]:
    ...
    if response is None or response.status_code != 200 or len(response.content) > MAX_RESPONSE_BYTES:
        status_code = response.status_code if response is not None else "missing"
        raise ValueError(f"transcription failed for {audio.name}: HTTP {status_code}")
...
def _align_with_bounded_padding(
    ...
    *,
    ...
    candidates = (
        ...
        if allow_padding_reduction
        else ((start_padding, end_padding),)
    )
...
def run(arguments: argparse.Namespace) -> dict[str, object]:
    ...
    if arguments.mode == "asr-only":
        raise ValueError("asr-only mode requires a dedicated collector")
                ...
                try:
                    hypothesis, applied_start_padding, applied_end_padding, padding_reduced = (
                        _align_with_bounded_padding(
                            ...
                            allow_padding_reduction=arguments.allow_padding_reduction,
                        )
                    )
                except ValueError as error:
...
def main() -> int:
    ...
    parser.add_argument("--allow-padding-reduction", action="store_true")
